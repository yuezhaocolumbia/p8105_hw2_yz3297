...
brfssnew_data=brfss_smart2010 %>%
  ...
  #clean the names, filter the overall health topic, drop other variables and turning from long to wide
...
There are `r count(distinct(brfssnew_data, locationdesc))` unique locations in the dataset.  There are `r count(distinct(brfssnew_data, locationabbr))` location abbreviations for states (or district), so we know all 50 states and Washington DC were represented.  

```{r}
which.max(table((brfssnew_data$locationabbr)))
```

The state which has the most observation is New Jersey. It appeared 32 times. 
